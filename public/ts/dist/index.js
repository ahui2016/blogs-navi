...
const titleArea = m('div').addClass('text-center').append(m('h1').append(util.LinkElem('/home', { text: '博客更新导航' })), m('div').text('批量检测博客更新，提供源代码可自建服务'));
...
const Hint = cc('div', { classes: 'Hint', children: [
        ...
        m('ul').append(m('li').text('首页默认展示有 feed (比如 RSS feed) 的博客'), m('li').text('在 Search 页面可搜索博客或按类别筛选博客。'), m('li').text('特别推荐阅读每个博客的 "about" 或 "关于" 页面, 都非常有趣.')),
    ] });
...
const naviBar = m('div').addClass('text-right').append(m(BtnShowCheckForm).on('click', event => {
    event.preventDefault();
...
}), util.LinkElem('/public/edit-blog.html', { text: 'Add', title: '添加博客', blank: true }).addClass('ml-2'), util.LinkElem('/public/search.html', { text: 'Search', blank: true }).addClass('ml-2'), m(HintBtn).addClass('ml-2').on('click', e => {
    e.preventDefault();
...
$('#root').append(titleArea, naviBar, m(CheckForm).hide(), m(DownloadDB).hide(), m(RevokeBtn).hide(), m(Loading).addClass('my-5'), m(Logs), m(Alerts).addClass('my-3'), m(Hint).addClass('my-3').hide(), m(BlogList).addClass('my-5'), m(Footer).hide());
init();
function init() {
    let body;
    ...
    util.ajax({ method: 'POST', url: url, alerts: Alerts, body: body }, resp => {
        blogs = resp;
...
function BlogItem(blog) {
    ...
    const self = cc('div', { id: blog.ID, classes: 'BlogItem text-grey', children: [
            m('div').addClass('BlogIDArea').append(span('[id:'), util.LinkElem('/public/blog-info.html?id=' + blog.ID, { text: blog.ID, title: '详细资料', blank: true }), span(']'), m('a').addClass('BlogCat ml-1')),
            m('div').addClass('BlogName').append(util.LinkElem(blog.Website, { text: blog.Name, blank: true })),
    ...
    self.init = () => {
        ...
        if (blog.Feed) {
            self.elem().find('.BlogIDArea').append(span(blog.Status).addClass('badge-grey ml-2').attr('title', '上次检测结果'));
            if (blog.Status != 'not yet') {
                self.elem().append(m('div').append(span(' checked at: ' + checkedAt), span(' updated at: ' + updatedAt)));
            }
            if (blog.ErrMsg) {
...
window.delete_blog_and_its_post = (blogID) => {
