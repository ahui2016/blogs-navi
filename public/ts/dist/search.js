...
const SearchForm = cc('form', { attr: { autocomplete: 'off' }, children: [
        ...
        m(SearchAlerts),
        m('div').addClass('text-center').append(m(SearchBtn).on('click', event => {
...
$('#root').append(titleArea, naviBar, m(Alerts), m(SearchForm), m(CatListArea).addClass('my-5'), m(RandomArea).addClass('my-5'), m(MoreBlogLists).addClass('my-5'), m(Footer));
init();
function init() {
    initCategories();
}
